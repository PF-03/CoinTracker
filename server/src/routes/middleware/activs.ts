...
import {
  /* filterActivos, */ getActivos,
  getActivosMayoresA,
  getMenoresA,
  getActivHistoryPrice,
} from "../../controllers/actives";
...
activos.get("/historyValue", async (req, res) => {
  try {
    ...
    const data: any = await getActivHistoryPrice(coinId, userId, vs_currency);
...
activos.get("/", async (req, res) => {
  ...
  if (name) {
    let foundName = activos_?.filter((e: any) =>
      e.name.toLowerCase().includes(name.toString().toLowerCase())
    );

    foundName.length ? (activos_ = foundName) : console.log("error");
  }
  if (maximo) {
